    ...
    clap() {
        axios.post('/clap/{{ $post->id }}')
            .then(response => {
                ...
                this.count = response.data.claps_count;
            })
            .catch(error => {
                console.error('Error:', error);
            });
    }
}" class="mt-6 border-t border-b p-2">
    <!-- Smile, breathe, and go slowly. - Thich Nhat Hanh -->
    <button
        class="flex gap-2 text-gray-500 hover:text-gray-700 focus:outline-none focus:ring-2 focus:ring-gray-300 rounded-lg p-2"
        @click="clap()">
        <template x-if="hasClapped">
            <svg xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 24 24" class="size-6">
                <path
                    d="M7.493 18.5c-.425 0-.82-.236-.975-.632A7.48 7.48 0 0 1 6 15.125c0-1.75.599-3.358 1.602-4.634.151-.192.373-.309.6-.397.473-.183.89-.514 1.212-.924a9.042 9.042 0 0 1 2.861-2.4c.723-.384 1.35-.956 1.653-1.715a4.498 4.498 0 0 0 .322-1.672V2.75A.75.75 0 0 1 15 2a2.25 2.25 0 0 1 2.25 2.25c0 1.152-.26 2.243-.723 3.218-.266.558.107 1.282.725 1.282h3.126c1.026 0 1.945.694 2.054 1.715a11.95 11.95 0 0 1-2.649 7.521c-.388.482-.987.729-1.605.729H14.23c-.483 0-.964-.078-1.423-.23l-3.114-1.04a4.501 4.501 0 0 0-1.423-.23h-.777ZM2.331 10.727a11.969 11.969 0 0 0-.831 4.398c0 .889 .825 .397 .914 .397H4v-5H3c-.832 .001 -1 .453 -1 .227Z" />
            </svg>
        </template>
        <template x-if="!hasClapped">
